...
import cn.aitenry.iims.common.model.vo.HolidayVO;
...
public class HolidayUtil {
    ...
    /**
     * 调用免费API查询全年工作日、周末、法定节假日、节假日、调休数据
     * 1、调用 <a href="https://api.apihubs.cn/holiday/get?size=500&year=2021">...</a> 查询全年日历（含周末）
     * 2、调用 <a href="https://timor.tech/api/holiday/year/2021">...</a> 查询全年节假日、调休
     */
    public static ArrayList<HolidayVO> getAllHolidayByYear(String year) throws IOException {
        ...
        allDayDataList.forEach((value) -> {
            ...
            Map<?, ?> valueTemp = (Map<?, ?>) value;
            String YEAR = valueTemp.get("year").toString();
            String MONTH = valueTemp.get("month").toString().replace(YEAR, "");
            String DAY = valueTemp.get("date").toString().replace(YEAR + MONTH, "");

            holidayVo.setData(YEAR + "-" + MONTH + "-" + DAY);
            String STATUS = "0";
            String msg = "工作日";
            if ("1".equals(valueTemp.get("weekend").toString())) {
                STATUS = "1";
                msg = "周末";
            }
